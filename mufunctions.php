...
function isActivatedUser($user, $pass) {
	global $ydb;
	$table = YOURLS_DB_TABLE_USERS;
	$pass = md5($pass);
	$results = $ydb->get_results("select verified from `$table` where `user_email` = '$user' AND user_password = '$pass'");
	if(!empty($results)) {
		return $results[0]->verified;
	}
	return false;
}
function isVerifiedUser($code) {
	global $ydb;
	$table = YOURLS_DB_TABLE_USERS;

	$ydb->query("UPDATE `$table` SET verified = '1' where verification_code = '$code'");
